...
class ParseFirebaseData {
    ...
    fun getAllUser(dataSnapshot: DataSnapshot, uid : String): ArrayList<Friend> {
    ...
    fun getAllLastMessages(dataSnapshot: DataSnapshot, uid : String): ArrayList<ChatMessage> {
        ...
        for (wholeChatData in dataSnapshot.children) {
            ...
            for (oneTemp in tempMsgList) {
                if (uid.equals(oneTemp.receiver.id) || uid.equals(
                        oneTemp.senderId
                    )
                ) {
